...
pub fn render_snake_window(parent_height: i32, parent_width: i32) -> impl Renderable {
    ...
    maud! {
        (Raw::dangerously_create(r#"
            <script type="module">
                import {setupSnakeToolBar, setupSnakeWindowGrab} from "/assets/js/snake.js";
                setupSnakeToolBar();
                setupSnakeWindowGrab();
            </script>
        "#))
        div id="snake-canvas-container" class="absolute" style={ "top:" (top) "px; left:" (left) "px;" } {
            div id="snake-toolbar" class="flex justify-between items-center bg-zinc-950 px-3 h-12 select-none" {

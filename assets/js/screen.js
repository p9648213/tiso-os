  ...
  main.addEventListener("contextmenu", (event) => {
    ...
    if (desktopItem) {
      ...
      for (const contextItem of CONTEXT_MENU_SCREEN) {
        ...
        if (contextItem === "Display Setting") {
          const main = document.querySelector("main");
          menuForm.addEventListener("mouseup", () => {
            htmx.ajax(
              "GET",
              `/read/setting/display/${main.clientHeight}/${main.clientWidth}`,
              {
                target: "body",
                swap: "beforeend",
              }
    ...
    if (draggedItem && event.target && draggedItem.id && event.target.id) {
      ...
      if (dragElement.innerHTML.trim() && !dropElement.innerHTML.trim()) {
        ...
        if (response.ok) {
          ...
          MessageBox.error(
            "Error",
            "Failed to move item: " + (await response.text())
          );
        }
      ...
      if (itemType === "file") {
        ...
        if (fileType === "ThisPC") {
          htmx
            .ajax(
              "GET",
              `/read/folder/explorer/Root/${itemId}/${main.clientHeight}/${main.clientWidth}/true/-1`,
              {
                target: "body",
                swap: "beforeend",
              }
